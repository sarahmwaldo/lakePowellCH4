...
lakePowellMeans2<-read.csv(paste(myWD, "output/lakePowellFluxes20190322.csv", sep="/"))
...
LP.littoral.summary<-LP.littoral%>%
  summarize(meanCH4lit = mean(CH4trate.mean),
            sdCH4lit = exp(sd(log(CH4trate.mean))), #log is the natural log transformation, since the msmts are log-normally distributed
            seCH4lit = sdCH4lit/8,
            fracErrlit = seCH4lit/meanCH4lit*100)
...
ErrCH4.lit.kg.y<-LP.littoral.summary$fracErrlit/100*totCH4.lit.kg.y
...
LP.openWater.summary<-LP.openWater%>%
  summarize(meanCH4ow = mean(CH4trate.mean),
            sdCH4ow = exp(sd(log(CH4trate.mean))),
            seCH4ow = sdCH4ow/20,
            fracErrow = seCH4ow/meanCH4ow*100)
...
ErrCH4.ow.kg.y<-LP.openWater.summary$fracErrlit/100*totCH4.ow.kg.y

totCH4.powell.kg.y<-sum(totCH4.lit.kg.y, totCH4.ow.kg.y)
totCH4err.powell.kg.y<-sqrt(sum(ErrCH4.lit.kg.y^2, ErrCH4.ow.kg.y^2))

totFracErr<-totCH4err.powell.kg.y/totCH4.powell.kg.y*100 #as a percent, 6.5%
...
powell.areal.ch4.co2e.err<-powell.areal.ch4.co2e*totFracErr/100 #+/- 15.5
...
powell.areal.CO2err.g.m2.y #+/- 8.7

totalArealEmissions<-sum(totCO2.g.m2.y, powell.areal.ch4.co2e)
totalArealEmissions #324.95
totArealError<-sqrt(sum(powell.areal.ch4.co2e.err^2, totCO2err.g.m2.y^2))
totArealError #+/- 17.7
...
powell.ef.err.ch4.co2e<-powell.ef.ch4.co2e*totFracErr/100
powell.ef.err.ch4.co2e #+/- 1.4
...
totEF.err<-sqrt(sum(powell.ef.err.co2.co2e^2, powell.ef.err.ch4.co2e^2))
